...
func (d *lxc) Update(args db.InstanceArgs, userRequested bool) error {
	...
	if isRunning {
		...
		d.logger.Info("Update called with changedConfig", logger.Ctx{
			"keys": strings.Join(changedConfig, ", "),
		})
		...
		for _, key := range changedConfig {
			...
			if key == "raw.apparmor" || key == "security.nesting" {
				...
				// Debug
				d.logger.Debug("Found OOM priority in changedConfig", logger.Ctx{
					"key":       key,
					"old_value": oldExpandedConfig["limits.memory.oom_priority"],
					"new_value": d.expandedConfig["limits.memory.oom_priority"],
				})
